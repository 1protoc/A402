# x402 vs A402 Side-by-Side Demo
...
- A402 Seller: `scripts/demo/a402-seller.js`
- A402 Buyer: `scripts/demo/a402-buyer.js`
...
The A402 scripts follow the repo quickstart shape:

- Seller: `a402-express` `paymentMiddleware` + `A402ResourceServer`
- Buyer: `a402-sdk` `wrapFetchWithPayment`
...
## A402 Private Vault Payment

Terminal 1:

```bash
A402_PUBLIC_ENCLAVE_URL="http://127.0.0.1:3100" yarn demo:a402-seller
```

Terminal 2:

```bash
A402_PUBLIC_ENCLAVE_URL="http://127.0.0.1:3100" \
A402_SELLER_URL="http://127.0.0.1:4022/weather" \
...
- The first visible transaction is `buyer token account -> A402 vault`.
- The seller payout is `A402 vault -> seller token account` after batching.
...
```bash
export A402_BATCH_WINDOW_SEC=60
export A402_MIN_ANONYMITY_WINDOW_SEC=60
export A402_MIN_BATCH_PROVIDERS=1
```
...
- `GET /a402/weather` for A402 private vault settlement
- `GET /.well-known/a402.json` for public route and attestation metadata

On the seller EC2:

```bash
git clone <repo-url> a402
cd a402
yarn install --frozen-lockfile
npm --prefix middleware run build
npm --prefix sdk run build

export A402_SELLER_HOST="0.0.0.0"
export A402_SELLER_PORT="8080"
export A402_SELLER_PUBLIC_URL="https://seller.example.com"

export SELLER_WALLET="<seller wallet owner address>"
export A402_PUBLIC_ENCLAVE_URL="https://api.demo.a402fi.com"
export A402_NETWORK="solana:devnet"
export A402_USDC_MINT="<facilitator Devnet USDC mint>"
export A402_DEMO_PAYMENT_AMOUNT="1100000"
...
```bash
A402_X402_SELLER_URL="https://seller.example.com/x402/weather" \
yarn demo:x402-buyer

A402_PUBLIC_ENCLAVE_URL="https://api.demo.a402fi.com" \
A402_SELLER_URL="https://seller.example.com/a402/weather" \
yarn demo:a402-buyer
```
...
- `A402_USDC_MINT`
- `A402_SOLANA_RPC_URL` or `ANCHOR_PROVIDER_URL` using an authenticated
  Solana Devnet RPC such as Alchemy
- `ANCHOR_WALLET`
- either `A402_USDC_MINT_AUTHORITY_WALLET` or
  `A402_NITRO_E2E_SOURCE_TOKEN_ACCOUNT`

For a long-running EC2 process, use the same environment variables in systemd:

```ini
[Unit]
Description=A402 public seller demo
After=network-online.target

[Service]
WorkingDirectory=/opt/a402
Environment=A402_SELLER_HOST=0.0.0.0
Environment=A402_SELLER_PORT=8080
Environment=A402_SELLER_PUBLIC_URL=https://seller.example.com
Environment=SELLER_WALLET=<seller wallet owner address>
Environment=A402_PUBLIC_ENCLAVE_URL=https://api.demo.a402fi.com
Environment=A402_NETWORK=solana:devnet
Environment=A402_USDC_MINT=<facilitator Devnet USDC mint>
...
`https://api.demo.a402fi.com` is the A402 facilitator URL, not a seller API.
...
a summarized view at `/.well-known/a402.json`.

For a production-grade Buyer verification policy, pin expected PCR values and
signing identity in the Buyer environment, then let `a402-sdk` verify the
...
- Seller route metadata: `GET <seller>/.well-known/a402.json`
...
There is no special "shooting mode" in these demo scripts. A402 payout timing
