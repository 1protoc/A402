...
- **Client**: you ship an agent, call any `a402-svm-v1` API with `a402-sdk`, and the vault handles the rest.
- **Provider**: you ship a paid API, plug in `a402-express`, and settle through the facilitator instead of direct on-chain transfers.
...
- The URL of a running A402 facilitator (your own Nitro deployment, or a public one)
- The facilitator's `attestationPolicyHash` and the PCR pinning values for its EIF

```bash
# export these once
export A402_FACILITATOR_URL="https://enclave.example.com"
export A402_NETWORK="solana:devnet"
export A402_USDC_MINT="<devnet USDC mint>"
```

## 2. Buyer: call a paid API

```bash
yarn add a402-sdk @solana/kit
```

No A402 API key or account registration is required. A buyer needs a funded Solana signer, a trusted facilitator, and an optional `autoDeposit` hook if they want x402-style on-demand top-ups.

```ts
import { A402Client, wrapFetchWithPayment } from "a402-sdk";
...
const client = new A402Client({
  signer,
  network: process.env.A402_NETWORK!,
  trustedFacilitators: [process.env.A402_FACILITATOR_URL!],
  autoDeposit: {
  ...
  nitroAttestation: {
    policy: {
      version: 1,
      pcrs: {
        "0": process.env.A402_PCR0!,
        "1": process.env.A402_PCR1!,
        "2": process.env.A402_PCR2!,
        "3": process.env.A402_PCR3!,
        "8": process.env.A402_PCR8!,
      },
      eifSigningCertSha256: process.env.A402_EIF_CERT_SHA256!,
      kmsKeyArnSha256: process.env.A402_KMS_KEY_ARN_SHA256!,
...
If `autoDeposit` is disabled, the buyer must already have spendable balance in the A402 vault.

## 3. Seller: serve a paid API

```bash
yarn add a402-express express
```
...
const facilitator = new A402FacilitatorClient({
  url: process.env.A402_FACILITATOR_URL!,
  assetMint: process.env.A402_USDC_MINT!,
});

const resourceServer = new A402ResourceServer(facilitator).register(
  process.env.A402_NETWORK!,
  new A402ExactScheme()
);
...
That is the default seller integration: install the package, point it at a deployed A402 facilitator, choose a route, price, network, and receiving wallet.
...
| -------------------------- | ------- | ----------------------------------- |
| `MIN_ANONYMITY_WINDOW_SEC` | `300`   | `A402_MIN_ANONYMITY_WINDOW_SEC` |
| `BATCH_WINDOW_SEC`         | `120`   | `A402_BATCH_WINDOW_SEC`         |
| `MIN_BATCH_PROVIDERS`      | `2`     | `A402_MIN_BATCH_PROVIDERS`      |
...
For a public demo you can set `A402_BATCH_WINDOW_SEC=60`,
`A402_MIN_ANONYMITY_WINDOW_SEC=60`, and
`A402_MIN_BATCH_PROVIDERS=1` so viewers can see the payout resolve. Do not
