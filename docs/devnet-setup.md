...
Set `A402_ADMIN_AUTH_TOKEN` in `.env.devnet.local` when
`A402_ENABLE_PROVIDER_REGISTRATION_API=1` or `A402_ENABLE_ADMIN_API=1`.
Single-provider smoke tests that need immediate on-chain payout must also set
`A402_ALLOW_ADMIN_PRIVACY_BYPASS_BATCH=1`; keep it unset or `0` for public
runtime.
