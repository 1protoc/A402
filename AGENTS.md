...
Implementation follows a phased approach. Always check which phase is currently active before implementing.
...
Record implementation progress and important decisions in `.Codex/MEMORY.md`.

- When each phase is completed, record what was implemented, test results, and remaining work.
- Also record important design decisions, such as library choices or deviations from the design documents.
- At the start of a new conversation, read `.Codex/MEMORY.md` to understand the previous implementation status.
