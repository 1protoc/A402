...
use crate::state::{
    ChannelState, ClientBalance, MultiChainProviderCredit, PendingWithdrawal, ProviderCredit,
    ProviderRegistration, Reservation, SettlementBatchInfo, SettlementRecord,
};
...
struct StateSnapshot {
    ...
    #[serde(default)]
    multi_chain_provider_credits: Vec<MultiChainProviderCredit>,
...
impl SnapshotManager {
    pub fn from_env(
        ...
        let every_n_events = read_env_u64("A402_SNAPSHOT_EVERY_N_EVENTS")
            .unwrap_or(DEFAULT_SNAPSHOT_EVERY_N_EVENTS);
        let retain_count = read_env_usize("A402_SNAPSHOT_RETAIN_COUNT")
...
async fn capture_snapshot(
    ...
    let mut multi_chain_provider_credits = state
        .vault
        .multi_chain_provider_credits
        .iter()
        .map(|entry| entry.value().clone())
        .collect::<Vec<_>>();
    multi_chain_provider_credits.sort_by(|a, b| a.provider_id.cmp(&b.provider_id));
...
async fn apply_snapshot(state: &Arc<AppState>, snapshot: &StateSnapshot) -> Result<(), String> {
    ...
    state.vault.multi_chain_provider_credits.clear();
    ...
    for provider_credit in &snapshot.multi_chain_provider_credits {
        state
            .vault
            .multi_chain_provider_credits
            .insert(provider_credit.provider_id.clone(), provider_credit.clone());
...
mod tests {
    ...
    async fn snapshot_capture_and_apply_roundtrip() {
        ...
        state.vault.multi_chain_provider_credits.insert(
            "eth-provider".to_string(),
            MultiChainProviderCredit {
                provider_id: "eth-provider".to_string(),
                network: "eip155:1".to_string(),
                asset_id: "eth".to_string(),
                settlement_address: "0x1111111111111111111111111111111111111111".to_string(),
                credited_amount: 88,
                oldest_credit_at: 124,
                settlement_ids: vec!["set-2".to_string()],
            },
        );
