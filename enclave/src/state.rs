...
/// Provider credit for non-Solana chains. These credits are settled by a
/// chain-specific adapter instead of the Solana `settle_vault` instruction.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MultiChainProviderCredit {
    pub provider_id: String,
    pub network: String,
    pub asset_id: String,
    pub settlement_address: String,
...
pub struct VaultState {
    ...
    pub multi_chain_provider_credits: DashMap<String, MultiChainProviderCredit>,
...
impl VaultState {
    pub fn new(
        ...
        Self {
            ...
            multi_chain_provider_credits: DashMap::new(),
    ...
    pub fn settle_payment(
        ...
        if crate::chain_adapter::is_solana_network(&provider.network) {
            let mut credit =
                self.provider_credits
            ...
            let mut credit = self
                ...
                .entry(provider_id.to_string())
                .or_insert_with(|| MultiChainProviderCredit {
                    provider_id: provider_id.to_string(),
                    network: provider.network.clone(),
                    asset_id: provider.asset_id.clone(),
                    settlement_address: provider.settlement_address.clone(),
                    credited_amount: 0,
                    oldest_credit_at: now,
                    settlement_ids: Vec::new(),
                });
    ...
    pub fn rollback_settle_payment(
        ...
        if let Some(provider) = self.providers.get(provider_id) {
            if crate::chain_adapter::is_solana_network(&provider.network) {
                ...
                if remove_credit {
                    self.multi_chain_provider_credits.remove(provider_id);
                }
            }
        }
    ...
    pub fn apply_batch_confirmation(
        ...
        for settlement_id in settlement_ids {
            ...
            for mut credit in self.multi_chain_provider_credits.iter_mut() {
                if credit.settlement_ids.iter().any(|sid| sid == settlement_id) {
                    affected_provider_ids.insert(credit.provider_id.clone());
                    credit.settlement_ids.retain(|sid| sid != settlement_id);
                    break;
        ...
        let multichain_provider_ids_to_remove: Vec<String> = affected_provider_ids
            .iter()
            .filter_map(|provider_id| {
                self.multi_chain_provider_credits
                    .get(provider_id)
                    .and_then(|credit| {
                        if credit.credited_amount == 0 || credit.settlement_ids.is_empty() {
                            Some(provider_id.clone())
            ...
            .collect();
        for provider_id in multichain_provider_ids_to_remove {
            self.multi_chain_provider_credits.remove(&provider_id);
            affected_provider_ids.remove(&provider_id);
        }
        ...
        for provider_id in affected_provider_ids {
            ...
            if let Some(mut credit) = self.multi_chain_provider_credits.get_mut(&provider_id) {
                if let Some(oldest_credit_at) = credit
                    .settlement_ids
                    .iter()
                    .filter_map(|settlement_id| {
