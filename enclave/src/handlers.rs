...
use crate::batch;
use crate::chain_adapter::{parse_network, SettlementTarget};
...
const SUPPORTED_PAYMENT_SCHEMES: &[&str] = &[
    "a402-svm-v1",
    "a402-v1",
    "a402-evm-v1",
    "a402-btc-v1",
...
fn is_supported_payment_scheme(scheme: &str) -> bool {
    SUPPORTED_PAYMENT_SCHEMES.contains(&scheme)
}
...
pub async fn get_attestation(State(state): State<Arc<AppState>>) -> Json<AttestationResponse> {
    ...
    Json(AttestationResponse {
        ...
        tee_platform: runtime_attestation.tee_platform,
...
fn derive_open_provider_id(network: &str, asset_mint: &str, pay_to: &str) -> String {
    ...
    hasher.update(b"A402-OPEN-PROVIDER-V1\n");
...
async fn ensure_open_provider_registered(
    ...
    let target = SettlementTarget::parse(&payload.network, &payload.asset_mint, &payload.pay_to)
        .map_err(|_| EnclaveError::InvalidPaymentDetails)?;
    let chain_kind = parse_network(&payload.network)
        .map_err(|_| EnclaveError::InvalidPaymentDetails)?
        .kind;
    let settlement_token_account = target
        .solana_settlement_token_account
        .unwrap_or_else(Pubkey::default);
    let asset_mint = target.solana_asset_mint.unwrap_or_else(Pubkey::default);
    let registration = crate::state::ProviderRegistration {
        ...
        settlement_address: target.settlement_address.clone(),
        asset_id: target.asset_id.clone(),
        chain_kind: format!("{chain_kind:?}").to_ascii_lowercase(),
    };
    ...
    state
        ...
        .append(WalEntry::ProviderRegistered {
            ...
            api_key_hash: None,
            mtls_cert_fingerprint: None,
            settlement_address: Some(target.settlement_address.clone()),
            asset_id: Some(target.asset_id.clone()),
            chain_kind: Some(format!("{chain_kind:?}").to_ascii_lowercase()),
        })
        .await
...
pub async fn post_verify(
    ...
    // 1. Validate scheme
    if !is_supported_payment_scheme(&payload.scheme) {
        return Err(EnclaveError::InvalidScheme);
    }
    let details_scheme = payment_details
        .get("scheme")
        .and_then(|value| value.as_str())
        .ok_or(EnclaveError::InvalidScheme)?;
    if details_scheme != payload.scheme || !is_supported_payment_scheme(details_scheme) {
        return Err(EnclaveError::InvalidScheme);
    }
    ...
    let registered_pay_to = if provider.settlement_address.is_empty() {
        ...
        provider.settlement_address.clone()
    };
    let registered_asset = if provider.asset_id.is_empty() {
        provider.asset_mint.to_string()
    } else {
        provider.asset_id.clone()
    };
    let payload_target =
        SettlementTarget::parse(&payload.network, &payload.asset_mint, &payload.pay_to)
            .map_err(|_| EnclaveError::InvalidPaymentDetails)?;
    if payload_target.settlement_address != registered_pay_to {
        return Err(EnclaveError::PayToMismatch);
    }
    if payload_target.asset_id != registered_asset {
        return Err(EnclaveError::AssetMintMismatch);
    }
...
pub async fn post_register_provider(
    ...
    let target =
        SettlementTarget::parse(&req.network, &req.asset_mint, &req.settlement_token_account)
            .map_err(|error| {
                EnclaveError::Internal(format!("Invalid settlement target: {error}"))
            })?;
    let chain_kind = parse_network(&req.network)
        .map_err(|error| EnclaveError::Internal(format!("Invalid network: {error}")))?
        .kind;
    let settlement_token_account = target
        .solana_settlement_token_account
        .unwrap_or_else(Pubkey::default);
...
fn admin_privacy_bypass_enabled() -> bool {
    matches!(
        env::var("A402_ALLOW_ADMIN_PRIVACY_BYPASS_BATCH")
            .ok()
            .as_deref(),
