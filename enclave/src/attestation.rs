...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
struct A402SevSnpAttestationEnvelope<'a> {
    version: u32,
    platform: &'static str,
    report_b64: &'a str,
    vault_config: String,
...
enum AttestationMode {
    ...
    SevSnpStatic { report_b64: String },
}
...
pub fn build_static_sev_snp_attestation(report_b64: String) -> AttestationBundle {
    AttestationBundle {
        document_b64: report_b64,
        is_local_dev: false,
        recipient_public_key_pem: None,
    }
}
...
pub fn resolve_attestation_policy_hash_from_env() -> Result<[u8; 32], String> {
    ...
    if should_use_sev_snp_attestation() {
        return resolve_sev_snp_policy_hash_from_env();
    }
    ...
    let eif_signing_cert_sha256 =
        std::env::var("A402_EIF_SIGNING_CERT_SHA256").map_err(|_| {
    ...
    let kms_key_arn_sha256 = match std::env::var("A402_KMS_KEY_ARN_SHA256") {
        ...
        Err(_) => {
            let key_arn = std::env::var("A402_KMS_KEY_ARN").map_err(|_| {
                "A402_KMS_KEY_ARN or A402_KMS_KEY_ARN_SHA256 must be set for Nitro policy derivation"
                    .to_string()
            })?;
            hex::encode(sha2::Sha256::digest(key_arn.as_bytes()))
        }
    };
    let protocol = std::env::var("A402_ATTESTATION_PROTOCOL")
        .unwrap_or_else(|_| "a402-svm-v1".to_string());
...
impl AttestationProvider {
    pub fn from_bootstrap_bundle(
        ...
        let mode = if should_use_sev_snp_attestation() {
            if bundle.document_b64.trim().is_empty() {
                return Err(
                    "SEV-SNP attestation requires A402_SEV_SNP_ATTESTATION_REPORT_B64"
                        .to_string(),
                );
            }
            AttestationMode::SevSnpStatic {
                report_b64: bundle.document_b64,
            }
        } else if should_use_dynamic_nitro_attestation() {
    ...
    pub fn runtime_attestation(
        ...
        match &self.mode {
            AttestationMode::LocalDev {
                ...
                is_local_dev: true,
                tee_platform: "local-dev".to_string(),
                snapshot_seqno,
            ...
            AttestationMode::NitroDynamic => Ok(RuntimeAttestation {
                ...
                is_local_dev: false,
                tee_platform: "aws-nitro".to_string(),
                snapshot_seqno,
                tls_public_key_sha256: self
                    .tls_binding
                    .as_ref()
            ...
            AttestationMode::SevSnpStatic { report_b64 } => Ok(RuntimeAttestation {
                document_b64: build_runtime_sev_snp_attestation(
                    vault_config,
                    vault_signer,
                    attestation_policy_hash,
                    snapshot_seqno,
                ...
                is_local_dev: false,
                tee_platform: "amd-sev-snp".to_string(),
                snapshot_seqno,
                tls_public_key_sha256: self
                    .tls_binding
                    .as_ref()
...
fn build_runtime_sev_snp_attestation(
    vault_config: Pubkey,
    vault_signer: Pubkey,
    attestation_policy_hash: [u8; 32],
    snapshot_seqno: u64,
    ...
    BASE64
        .decode(report_b64)
        .map_err(|error| format!("A402_SEV_SNP_ATTESTATION_REPORT_B64 must be base64: {error}"))?;
    let issued_at = Utc::now();
    let expires_at = issued_at + Duration::minutes(10);
    let envelope = A402SevSnpAttestationEnvelope {
        version: 1,
    ...
    serde_json::to_vec(&envelope)
        ...
        .map_err(|error| format!("failed to serialize SEV-SNP attestation envelope: {error}"))
}
...
fn should_use_dynamic_nitro_attestation() -> bool {
    if matches!(
        tee_platform_from_env(),
        Some(TeePlatform::LocalDev | TeePlatform::AmdSevSnp)
    ) {
        return false;
    }
...
pub fn should_use_sev_snp_attestation() -> bool {
    matches!(tee_platform_from_env(), Some(TeePlatform::AmdSevSnp))
        || std::env::var("A402_SEV_SNP_ATTESTATION_REPORT_B64").is_ok()
}

fn tee_platform_from_env() -> Option<TeePlatform> {
    let value = std::env::var("A402_TEE_PLATFORM").ok()?;
    match value.trim().to_ascii_lowercase().as_str() {
        "local" | "local-dev" | "dev" => Some(TeePlatform::LocalDev),
        "nitro" | "aws-nitro" | "aws_nitro" => Some(TeePlatform::AwsNitro),
        "sev-snp" | "sev_snp" | "amd-sev-snp" | "amd_sev_snp" => {
...
fn resolve_sev_snp_policy_hash_from_env() -> Result<[u8; 32], String> {
    let measurement = read_required_hex_env("A402_SEV_SNP_MEASUREMENT_HEX")?;
    let protocol =
    ...
    let mut fields = serde_json::Map::from_iter([
        ...
        (
            "platform".to_string(),
            JsonValue::String("amd-sev-snp".to_string()),
        ),
        ("measurement".to_string(), JsonValue::String(measurement)),
        ("protocol".to_string(), JsonValue::String(protocol)),
    ]);

    for (env_name, json_key) in [
        ("A402_SEV_SNP_HOST_DATA_HEX", "hostData"),
        ("A402_SEV_SNP_REPORT_DATA_HEX", "reportData"),
        ("A402_SEV_SNP_FAMILY_ID_HEX", "familyId"),
        ("A402_SEV_SNP_IMAGE_ID_HEX", "imageId"),
        ("A402_SEV_SNP_POLICY_HEX", "policy"),
        ("A402_SEV_SNP_VMPL", "vmpl"),
    ] {
        if let Ok(value) = std::env::var(env_name) {
            let normalized = if env_name == "A402_SEV_SNP_VMPL" {
                value.trim().to_string()
            } else {
                normalize_hex(&value)?
            };
            fields.insert(json_key.to_string(), JsonValue::String(normalized));
        }
    }

    let canonical = canonical_json(&JsonValue::Object(fields));
    let digest = sha2::Sha256::digest(canonical.as_bytes());
    let mut out = [0u8; 32];
    out.copy_from_slice(&digest);
    Ok(out)
...
fn read_required_hex_env(name: &str) -> Result<String, String> {
    let value = std::env::var(name)
        .map_err(|_| format!("{name} must be set for SEV-SNP policy derivation"))?;
    normalize_hex(&value)
}
