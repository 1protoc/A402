...
use crate::multichain_settlement::{submit_multichain_batches, MultiChainSettlementEntry};
...
impl BatchPrivacyConfig {
    pub fn from_env() -> Self {
        ...
        if let Some((name, value)) =
            first_env_value(&["A402_BATCH_WINDOW_SEC", "BATCH_WINDOW_SEC"])
        {
        ...
        if let Some((name, value)) =
            first_env_value(&["A402_MIN_BATCH_PROVIDERS", "MIN_BATCH_PROVIDERS"])
        {
...
async fn fire_batch_at(
    ...
    let multichain_prepared = prepare_multichain_batch(state, now, mode).await?;
    if prepared.is_empty() && multichain_prepared.is_empty() {
        return Ok(BatchSubmitResult::no_op());
    }
    ...
    if chunks.is_empty() && multichain_prepared.is_empty() {
        return Ok(BatchSubmitResult::no_op());
    }
    ...
    let submitted_entries: Vec<&PreparedSettlement> = chunks
        ...
        .collect();
    let solana_provider_count = submitted_entries
        .iter()
        .map(|entry| entry.provider_id.clone())
        .collect::<HashSet<_>>()
        .len();
    let multichain_provider_count = multichain_prepared
        .iter()
        .map(|entry| entry.provider_id.clone())
        .collect::<HashSet<_>>()
        .len();
    let provider_count = solana_provider_count + multichain_provider_count;
    let total_amount: u64 = submitted_entries
        .iter()
        .map(|entry| entry.settlement.amount)
        .sum::<u64>()
        .saturating_add(multichain_prepared.iter().map(|entry| entry.amount).sum::<u64>());
    let settlement_count = submitted_entries.len() + multichain_prepared.len();
        ...
        for receipt in receipts {
            ...
            let tx_signature = format!("{:?}:{}:{}", receipt.chain_kind, receipt.network, receipt.tx_id);
            {
                ...
                state
                    ...
                    .await
                    .map_err(|error| {
                        format!("failed to append multichain BatchConfirmed WAL entry: {error}")
                    })?;
                state
                    .vault
            ...
            for provider_id in multichain_prepared
                .iter()
                .filter(|entry| settlement_ids.contains(&entry.settlement_id))
                .map(|entry| entry.provider_id.clone())
                .collect::<HashSet<_>>()
            {
                if let Err(error) = retry_provider_receipt_mirror(state, &provider_id, 3).await {
                    warn!(
                        ...
                        batch_id,
                        error = %error,
                        "Failed to mirror multichain provider receipt after batch confirmation"
                    );
                }
            }

            tx_signatures.push(tx_signature);
        }
    }
...
async fn prepare_multichain_batch(
    state: &Arc<AppState>,
    batch_timestamp: i64,
    mode: BatchBuildMode,
) -> Result<Vec<MultiChainSettlementEntry>, String> {
    let batch_privacy = state.batch_privacy;
    let provider_snapshots: Vec<(String, Vec<String>)> = state
        .vault
        .multi_chain_provider_credits
        .iter()
    ...
    for (provider_id, settlement_ids) in provider_snapshots {
        let timestamped: Vec<(String, i64, u64)> = settlement_ids
            .iter()
            .filter_map(|sid| {
                state
                    .vault
                    .settlement_history
                    .get(sid)
                    .map(|record| (sid.clone(), record.timestamp, record.amount))
            })
            .collect();
        let Some(selection) =
    ...
    for (provider_id, sid) in select_batch_settlement_ids(cursors, MAX_SETTLEMENTS_PER_TX) {
        ...
        prepared.push(MultiChainSettlementEntry {
            settlement_id: sid,
            provider_id,
            network: provider.network,
            asset_id: provider.asset_id,
            settlement_address: provider.settlement_address,
