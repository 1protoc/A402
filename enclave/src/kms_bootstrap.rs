...
use crate::attestation::{
    ...
    build_static_nitro_attestation, build_static_sev_snp_attestation,
    should_use_sev_snp_attestation, AttestationBundle,
};
...
pub async fn bootstrap_materials(
    ...
    let sev_snp_report = std::env::var("A402_SEV_SNP_ATTESTATION_REPORT_B64").ok();
    if should_use_sev_snp_attestation() && sev_snp_report.is_none() {
        return Err(
            "A402_TEE_PLATFORM=sev-snp requires A402_SEV_SNP_ATTESTATION_REPORT_B64".to_string(),
        );
    }
    let nitro_document = if should_use_sev_snp_attestation() {
        None
    } else {
        match std::env::var("A402_NITRO_ATTESTATION_DOCUMENT_B64") {
    ...
    let attestation = if let Some(report_b64) = sev_snp_report {
        build_static_sev_snp_attestation(report_b64)
    } else if let Some(document_b64) = nitro_document {
