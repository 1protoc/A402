...
const ADMIN_TOKEN_HASH_ENV: &str = "A402_ADMIN_AUTH_TOKEN_SHA256";
const ADMIN_TOKEN_ENV: &str = "A402_ADMIN_AUTH_TOKEN";
    ...
    if let Some(expected_hash) = env_value(ADMIN_TOKEN_HASH_ENV) {
        ...
        let Ok(expected_hash_bytes) = hex::decode(expected_hash) else {
            return Err(json_error(
                ...
                "A402_ADMIN_AUTH_TOKEN_SHA256 must be 64 hex characters",
            ));
        };
