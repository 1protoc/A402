...
type NormalizedAutoDepositConfig = Required<
  Pick<A402AutoDepositConfig, "deposit">
> &
  Omit<A402AutoDepositConfig, "deposit">;
...
function isSupportedWireScheme(scheme: string): scheme is A402WireScheme {
  return (
    scheme === "a402-svm-v1" ||
    scheme === "a402-v1" ||
    scheme === "a402-evm-v1" ||
...
function signerAddress(signer: A402Signer): string {
  if (signer.address) {
  ...
  throw new Error("A402 signer public key is required");
}
...
async function signMessage(
  ...
  if (signer.signMessages) {
    ...
    if (!signature) {
      throw new Error("A402 signer did not return a message signature");
    }
  ...
  throw new Error("A402 signer must provide signMessage or secretKey");
}
...
function parseAmountToAtomic(
  ...
  if (typeof amount === "number") {
    if (!Number.isFinite(amount) || amount < 0) {
      throw new Error("A402 amount must be a non-negative number");
    }
  ...
  if (!/^\d+(\.\d+)?$/.test(decimal)) {
    throw new Error(`Unsupported A402 amount: ${amount}`);
  }

  const [whole, fraction = ""] = decimal.split(".");
  if (fraction.length > decimals) {
    throw new Error(
      `A402 amount ${amount} has more than ${decimals} decimal places`
    );
  }
  return (
...
function normalizeAutoDeposit(
  autoDeposit: A402ClientConfig["autoDeposit"]
...
function verifyLocalAttestationDocument(
  ...
  if (localDocument.version !== 1 || localDocument.mode !== "local-dev") {
    throw new Error("Unsupported A402 local attestation document");
  }
  if (localDocument.vaultConfig !== attestation.vaultConfig) {
    throw new Error("A402 local attestation vaultConfig mismatch");
  }
  if (localDocument.vaultSigner !== attestation.vaultSigner) {
    throw new Error("A402 local attestation vaultSigner mismatch");
  }
  if (
    localDocument.attestationPolicyHash.toLowerCase() !==
    attestation.attestationPolicyHash.toLowerCase()
  ) {
    throw new Error("A402 local attestation policy hash mismatch");
  }
  if (
    localDocument.snapshotSeqno !== undefined &&
    localDocument.snapshotSeqno !== attestation.snapshotSeqno
  ) {
    throw new Error("A402 local attestation snapshotSeqno mismatch");
  }
}
...
async function verifyAttestedTlsBinding(
  ...
  if (!attestation.tlsPublicKeySha256) {
    ...
    throw new Error(
      "A402 non-local attestation is missing attested tlsPublicKeySha256"
    );
  }
  ...
  if (observed.toLowerCase() !== attestation.tlsPublicKeySha256.toLowerCase()) {
    throw new Error("A402 attested TLS public key mismatch");
  }
}
...
export class A402Client {
  ...
  private readonly nitroAttestation: A402ClientConfig["nitroAttestation"];
  private readonly attestationVerifier: A402ClientConfig["attestationVerifier"];
  private readonly attestationCache = new Map<string, AttestationResponse>();
  private readonly schemes: Array<{
    network: string;
    scheme: A402ExactScheme;
  }> = [];
  private defaultScheme?: A402ExactScheme;
  ...
  constructor(config: A402ClientConfig = {}) {
    ...
    if (config.signer) {
      const scheme = new A402ExactScheme(config.signer);
      if (config.network) {
  ...
  register(network: string, scheme: A402ExactScheme): this {
  ...
  async fetch(
    ...
    const selected = body.accepts
      ?.map((accept) => ({
        details: accept,
        scheme: isSupportedWireScheme(accept.scheme)
          ? this.findSchemeForNetwork(accept.network)
          : undefined,
      }))
      .find(
        (candidate) =>
          isSupportedWireScheme(candidate.details.scheme) && candidate.scheme
      );
    if (!selected?.scheme) {
      throw new Error(
        "No A402-compatible payment option for registered networks"
      );
    }
  ...
  private findSchemeForNetwork(network: string): A402ExactScheme | null {
  ...
  private async depositOnDemand(
    ...
    if (this.autoDeposit.maxDepositPerRequest !== undefined) {
      ...
      if (amountAtomic > maxDeposit) {
        throw new Error(
          `A402 autoDeposit exceeds policy: ${
            details.amount
          } > ${maxDeposit.toString()}`
        );
      }
  ...
  private async verifyAttestation(
    ...
    if (
      ...
      throw new Error(`Untrusted A402 facilitator: ${facilitatorUrl}`);
    }
    ...
    if (!response.ok) {
      throw new Error(`A402 attestation fetch failed: ${response.status}`);
    }
    ...
    if (isLocalDevAttestation) {
    ...
    } else if (attestation.teePlatform === "amd-sev-snp") {
      if (!this.attestationVerifier) {
        throw new Error(
          "SEV-SNP attestation requires a custom attestationVerifier"
        );
      }
      ...
      await verifyNitroAttestationDocument(attestation, {
        ...
        requireA402UserData:
          this.nitroAttestation.requireA402UserData ?? true,
      });
      if (this.attestationVerifier) {
        await this.attestationVerifier(attestation);
      }
      ...
      throw new Error(
        "A402Client requires nitroAttestation or attestationVerifier for non-local attestation"
      );
    }
...
export class A402ExactScheme {
  ...
  readonly wireScheme = "a402-v1";

  constructor(readonly signer: A402Signer) {}
}

export function wrapFetchWithA402Payment(
  fetchImpl: FetchLike,
  client: A402Client
): FetchLike {
