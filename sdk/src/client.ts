...
function toWeb3PublicKey(value: A402PublicKeyLike): PublicKey {
...
/**
 * A402 vault client SDK — deposit, withdraw, and direct enclave fetch.
 */
export class A402VaultClient {
  ...
  private nitroAttestation: A402VaultClientConfig["nitroAttestation"];
  private attestationVerifier: A402VaultClientConfig["attestationVerifier"];

  constructor(config: A402VaultClientConfig) {
    this.wallet = {
  ...
  async fetch(url: string, options?: RequestInit): Promise<Response> {
    ...
    const details = body.accepts?.find((a) =>
      [
        "a402-svm-v1",
        "a402-v1",
        "a402-evm-v1",
    ...
    if (!details) {
      throw new Error("No A402 payment option in 402 response");
    }
