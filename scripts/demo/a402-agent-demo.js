...
async function main() {
  ...
  const requestOrigin =
    process.env.A402_REQUEST_ORIGIN || "https://demo.a402.dev";
  ...
  printHeader("A402 privacy-first x402: private vault + batched settlement");
  logStep(1, requestSummary());
  logKV("Payment path", "agent -> A402 vault -> provider payout");
  ...
  for (let index = 0; index < providers.length; index += 1) {
    ...
    const providerResponse = buildDemoResponse({
      ...
      settlementMode: "a402-private-x402",
    });
