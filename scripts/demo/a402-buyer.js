...
function selectA402Details(paymentRequired, network) {
...
async function main() {
  ...
  const sellerUrl =
    process.env.A402_SELLER_URL ||
    `http://127.0.0.1:${
      process.env.A402_SELLER_PORT || 4022
    }${DEMO_ROUTE_PATH}`;
  ...
  if (preflight.status !== 402) {
    throw new Error(
      `Expected A402 seller to return 402 before payment, got ${preflight.status}`
    );
  }
  const preflightRequired = paymentRequiredFromResponse(preflight);
  const preflightDetails = selectA402Details(preflightRequired, network);
  if (!preflightDetails?.payTo) {
    throw new Error(
      "A402 402 response did not include a payTo token account"
    );
  }
  ...
  const recordingFetch = async (input, init) => {
    ...
    if (response.status === 402) {
      ...
      const details = paymentRequired?.accepts?.find(
        (accept) => accept.scheme === "a402-svm-v1"
      );
      if (details) {
  ...
  printHeader("A402 Buyer");
  logKV(
    "Docs pattern",
    "a402-sdk wrapFetchWithPayment + A402ExactScheme"
  );
  ...
  logKV("Vault token account", requireEnv("A402_VAULT_TOKEN_ACCOUNT"));
  ...
  const client = new A402Client({
    ...
    autoDeposit: {
      ...
      deposit: async ({ amountAtomic, details, reason }) => {
        ...
        const depositTx = await depositIntoA402Vault({
          rpc,
          feePayer,
          client: buyer,
        ...
        await waitForA402Balance(facilitatorUrl, buyer, amountAtomic);
        deposits.push({
    ...
    ...buildA402ClientAttestationOptions(),
  }).register("solana:*", new A402ExactScheme(buyer));
  ...
  if (
    settlementId &&
    providerId &&
    process.env.A402_DEMO_SKIP_BATCH_WAIT !== "1"
  ) {
    settlementStatus = await waitForEndpoint(
      "A402 batched provider payout",
      async () => {
      ...
      Number(process.env.A402_DEMO_BATCH_WAIT_ATTEMPTS || 72),
      Number(process.env.A402_DEMO_BATCH_WAIT_DELAY_MS || 5000)
    ).catch((error) => ({
      ok: false,
      settlementId,
  ...
  logKV(
    "Vault token account",
    shortKey(requireEnv("A402_VAULT_TOKEN_ACCOUNT"))
  );
  logKV("Seller token account", shortKey(sellerTokenAccount));
  logKV("Visible now", "buyer token account -> A402 vault deposit");
  ...
  logKV(
    "Vault explorer",
    explorerAddress(requireEnv("A402_VAULT_TOKEN_ACCOUNT"))
  );
