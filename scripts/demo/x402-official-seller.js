...
async function main() {
  ...
  const network = process.env.A402_X402_NETWORK || DEFAULT_X402_NETWORK;
  const facilitatorUrl =
    process.env.A402_X402_FACILITATOR_URL || DEFAULT_X402_FACILITATOR_URL;
  const host = process.env.A402_X402_SELLER_HOST || "127.0.0.1";
  const port = Number(process.env.A402_X402_SELLER_PORT || 4021);
  ...
  app.use(
    paymentMiddleware(
      ...
      resourceServer,
      {
        appName: "A402 side-by-side official x402 seller",
        testnet: true,
      }
    )
  );
  ...
  console.log(`  A402_X402_SELLER_URL='${url}' yarn demo:x402-buyer`);
}
