...
const DEFAULT_A402_NETWORK = "solana:devnet";
...
function loadFourWayDemoEnv() {
  ...
  aliasEnv("A402_PROGRAM_ID", "A402_PROGRAM_ID");
  aliasEnv("A402_VAULT_CONFIG", "A402_VAULT_CONFIG");
  aliasEnv("A402_VAULT_TOKEN_ACCOUNT", "A402_VAULT_TOKEN_ACCOUNT");
  aliasEnv("A402_USDC_MINT", "A402_USDC_MINT");
  aliasEnv(
    "A402_ATTESTATION_POLICY_HASH_HEX",
    "A402_ATTESTATION_POLICY_HASH_HEX"
  );
  aliasEnv("A402_PUBLIC_ENCLAVE_URL", "A402_PUBLIC_ENCLAVE_URL");

  if (
    process.env.A402_NITRO_ALLOW_SELF_SIGNED_TLS === "1" ||
    process.env.A402_ALLOW_SELF_SIGNED_TLS === "1"
  ) {
...
function demoDepositAmount() {
  return readPositiveIntEnv(
    ...
    readPositiveIntEnv("A402_NITRO_E2E_DEPOSIT_AMOUNT", demoPaymentAmount())
  );
}
...
async function fetchPublicSellerMetadata(routeUrl) {
  ...
  const response = await fetch(`${parsed.origin}/.well-known/a402.json`, {
    cache: "no-store",
  }).catch(() => null);
  if (!response?.ok) {
    return null;
...
async function resolveDemoSeller({ createAta = true } = {}) {
  ...
  const sellerWallet =
    process.env.A402_DEMO_SELLER_WALLET ||
    process.env.SELLER_WALLET ||
    feePayer?.address ||
    null;
  if (!sellerWallet) {
    throw new Error(
      "Set A402_DEMO_SELLER_WALLET or SELLER_WALLET to the seller wallet owner address"
    );
  }
...
async function fetchA402Balance(facilitatorUrl, signer) {
...
async function waitForA402Balance(facilitatorUrl, signer, minimumAtomic) {
  return waitForEndpoint(
    "A402 vault balance sync",
    async () => {
      const body = await fetchA402Balance(facilitatorUrl, signer);
      return BigInt(body.free.toString()) >= BigInt(minimumAtomic)
        ? body
        : null;
    },
    readPositiveIntEnv("A402_DEMO_BALANCE_WAIT_ATTEMPTS", 120),
    readPositiveIntEnv("A402_DEMO_BALANCE_WAIT_DELAY_MS", 1000)
  );
}

async function depositIntoA402Vault({
  rpc,
  feePayer,
  client,
  clientTokenAccount,
...
function buildA402ClientAttestationOptions() {
  const pcrs = {};
  for (const index of ["0", "1", "2", "3", "8"]) {
    const value = process.env[`A402_PCR${index}`];
    if (value) {
      pcrs[index] = value;
    }
  }
  const hasPcrPolicy =
    ...
    process.env.A402_EIF_CERT_SHA256 &&
    process.env.A402_KMS_KEY_ARN_SHA256;
  ...
  return {
    attestationVerifier: async (attestation) => {
      if (attestation.vaultConfig !== requireEnv("A402_VAULT_CONFIG")) {
        throw new Error("A402 attestation vaultConfig mismatch");
      }
      if (
        attestation.attestationPolicyHash.toLowerCase() !==
        requireEnv("A402_ATTESTATION_POLICY_HASH_HEX").toLowerCase()
      ) {
      ...
      if (
        ...
        throw new Error("A402 attestation is expired");
      }
    },
  };
}
...
module.exports = {
  ...
  DEMO_MIME_TYPE,
  DEMO_ROUTE_PATH,
  buildA402ClientAttestationOptions,
  decodeJsonHeader,
  demoDepositAmount,
  demoPaymentAmount,
  demoWeatherResponse,
