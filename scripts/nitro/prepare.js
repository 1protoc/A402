...
function resolveAdminAuthTokenSha256() {
  if (process.env.A402_ADMIN_AUTH_TOKEN_SHA256) {
    return process.env.A402_ADMIN_AUTH_TOKEN_SHA256;
  }
  if (process.env.A402_ADMIN_AUTH_TOKEN) {
    return sha256hex(process.env.A402_ADMIN_AUTH_TOKEN);
  }
  return undefined;
}
...
async function main() {
  ...
  const protocol =
    args.protocol ||
    process.env.A402_ATTESTATION_PROTOCOL ||
    "a402-svm-v1";
  ...
  const enableAdminApi = process.env.A402_ENABLE_ADMIN_API || "0";
  ...
  if (
    ...
    throw new Error(
      [
        "A402_ADMIN_AUTH_TOKEN or A402_ADMIN_AUTH_TOKEN_SHA256",
  ...
  const enclaveEnv = {
    ...
    A402_ENCLAVE_INGRESS_PORT:
      process.env.A402_ENCLAVE_INGRESS_PORT || "5000",
    A402_ENCLAVE_EGRESS_PORT:
      process.env.A402_ENCLAVE_EGRESS_PORT || "5001",
    ...
    A402_WAL_PREFIX:
      process.env.A402_WAL_PREFIX || `wal/${vaultConfig}`,
    A402_WATCHTOWER_URL:
    ...
    A402_ENCLAVE_TLS_CERT_PATH:
      process.env.A402_ENCLAVE_TLS_CERT_PATH ||
      "/etc/a402/tls/server.crt",
    A402_ENCLAVE_TLS_KEY_PATH:
      process.env.A402_ENCLAVE_TLS_KEY_PATH ||
      "/etc/a402/tls/server.key",
    A402_ENABLE_PROVIDER_REGISTRATION_API: enableProviderRegistrationApi,
    A402_ENABLE_ADMIN_API: enableAdminApi,
    A402_ADMIN_AUTH_TOKEN_SHA256: adminAuthTokenSha256,
    A402_BATCH_WINDOW_SEC:
      firstEnvValue("A402_BATCH_WINDOW_SEC", "BATCH_WINDOW_SEC") || "120",
    A402_MIN_BATCH_PROVIDERS:
      firstEnvValue("A402_MIN_BATCH_PROVIDERS", "MIN_BATCH_PROVIDERS") ||
      "2",
    A402_MIN_ANONYMITY_WINDOW_SEC:
      firstEnvValue(
        "A402_MIN_ANONYMITY_WINDOW_SEC",
        "MIN_ANONYMITY_WINDOW_SEC"
      ) || "300",
    A402_AUTO_BATCH_MIN_PROVIDER_PAYOUT_ATOMIC:
      process.env.A402_AUTO_BATCH_MIN_PROVIDER_PAYOUT_ATOMIC || "1000000",
    A402_ALLOW_ADMIN_PRIVACY_BYPASS_BATCH:
      process.env.A402_ALLOW_ADMIN_PRIVACY_BYPASS_BATCH || "0",
  ...
  const watchtowerEnv = {
    ...
    A402_WATCHTOWER_POLL_SEC:
      process.env.A402_WATCHTOWER_POLL_SEC || "10",
    A402_PROGRAM_ID: program.programId.toBase58(),
    A402_VAULT_CONFIG: vaultConfig,
    A402_SOLANA_RPC_URL: process.env.A402_SOLANA_RPC_URL,
