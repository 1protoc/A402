...
function adminHeaders() {
  const token = process.env.A402_ADMIN_AUTH_TOKEN;
  if (!token) {
    throw new Error(
      "A402_ADMIN_AUTH_TOKEN is required for provider/register and admin/fire-batch"
    );
  }
...
async function main() {
  ...
  const requestOrigin =
    process.env.A402_REQUEST_ORIGIN || "http://127.0.0.1";
  ...
  const fireBatchRes = await postJson(
    enclaveUrl,
    "/v1/admin/fire-batch",
    process.env.A402_ALLOW_ADMIN_PRIVACY_BYPASS_BATCH === "1"
      ? { privacyBypass: true }
      : {},
    controlPlaneHeaders
  );
