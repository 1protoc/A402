...
import {
  A402Client,
  A402ExactScheme as BuyerA402ExactScheme,
} from "../sdk/src";
import type {
  AttestationResponse,
  PaymentDetails,
...
describe("a402 x402-compatible interface", () => {
  ...
  it("lets buyers pay from discovered 402 details with an @solana/kit signer", async () => {
    ...
    try {
      ...
      }).register("solana:*", new BuyerA402ExactScheme(signer));
  ...
  it("re-fetches a cached attestation whose details no longer match", async () => {
    // Regression for Codex #2: A402Client used to return a cached
