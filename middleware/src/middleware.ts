...
function wireSchemeForConfig(config: A402ProviderConfig): string {
  if (
    config.network.startsWith("eip155:") ||
    config.network.startsWith("ethereum:")
  ) {
    return "a402-evm-v1";
  }
  if (
    config.network.startsWith("bip122:") ||
    config.network.startsWith("bitcoin:")
  ) {
    return "a402-btc-v1";
  }
  return "a402-svm-v1";
...
/**
 * Express middleware implementing the A402 payment protocol.
 *
 * Flow:
...
function buildPaymentDetails(
  ...
  return {
    scheme: wireSchemeForConfig(config),
    network: config.network,
    amount,
    asset: {
      kind: config.assetKind ?? "spl-token",
