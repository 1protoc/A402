...
Express middleware that serves paid APIs through the A402 privacy-first x402 facilitator. Drop-in replacement for x402 sellers who want their settlements hidden behind a TEE-backed vault.
...
The default import path is the x402-like Seller middleware and uses `@solana/kit` plus `@solana-program/token` for Solana address handling. Advanced ASC helpers are available from `a402-express/asc`; that optional path requires the legacy Solana/Anchor dependencies.
...
That is the full seller integration: install the package, point it at a deployed A402 facilitator, choose a route, price, network, and receiving wallet.
